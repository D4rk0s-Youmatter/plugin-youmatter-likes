...
class ym_likes_Public
{
    ...
    {
        wp_enqueue_style($this->plugin_name . "_plublic", plugin_dir_url(__FILE__) . 'css/ym-likes-public.css', array(), $this->version, 'all');
    }
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script($this->plugin_name . "_public", plugin_dir_url(__FILE__) . 'js/ym-likes-public.js', array('jquery'), $this->version, true);
    ...
    public function set_posts_likes()
    {
        ...
        if (isset($_POST['ymli_post_id'])  && isset($_POST['ymli_likes']) && isset($_POST['ymli_likes_to_add'])) {

            // We transpose de vars for cleaner code below
            $post_id = $_POST['ymli_post_id'];
            $action = $_POST['ymli_likes'];
            $likes_to_add = $_POST['ymli_likes_to_add'];
            $meta_key = "ymli_likes";
            ...
            if ($action == 'like') {
                $newLikes = $likes_number + intval($likes_to_add);
    ...
    public function set_user_like($post_id, $action = "like", $user_id = null)
    {
        ...
        if (null !== $user_id) {
            ...
            $likes_meta = get_user_meta($user_id, "ymli_liked", true);
            ...
            // We update the data
            update_user_meta( $user_id, "ymli_liked", implode (",", $likes_meta) );

            $updated_meta = get_user_meta($user_id, "ymli_liked", true);        
            return $updated_meta;
        }
    ...
    public static function get_user_likes($user_id = null)
    {
        if (is_user_logged_in() OR !null == $user_id ) {
            $user_id = get_current_user_id();
            return explode(',', get_user_meta($user_id, 'ymli_liked', true) );
        } 
        else if( isset( $_COOKIE['ymli_liked'] ) ) {    
            return explode(',', $_COOKIE['ymli_liked']);
        }
    }
    ...
    {
        return stripslashes($_COOKIE['ymli_liked']);
    }
    /**
    ...
    {
        $cookie_data = array();
        if(isset($_COOKIE["ymli_liked"])) {
            $cookie_data = explode(',', stripslashes($_COOKIE["ymli_liked"]) ); 
            //return $cookie_data;
        ...
        setcookie('ymli_liked', implode (", ", $cookie_data), time()+7889231,'/');
    ...
    public static function is_item_liked($post_id = "")
    {
        ...
        $user_likes = ym_likes_Public::get_user_likes();
    ...
    public static function output_like_form($post_id = "")
    {
        ...
        $likes_number = 0;
        $likes_number_meta = get_post_meta($post_id, "ymli_likes", true);

        if (!empty($likes_number_meta)) {
            $likes_number = $likes_number_meta;
        }

        $is_item_liked = ym_likes_Public::is_item_liked($post_id);
        if($is_item_liked) {
            echo "<div class='ymli_container action__btn liked' data-post_id='" . $post_id . "'>";
        } else {
            echo "<div class='ymli_container action__btn notliked' data-post_id='" . $post_id . "'>";

        }
        echo "<span class='ymli_heart_container'>";
        echo "<span class='ymli_counter_container hidden'>";
        echo "</span>";

        include(plugin_dir_path(__FILE__) . "/assets/img/digit.svg");
        echo "</span>";
        echo "<b>" . $likes_number . "</b>";
        echo "</div>";
    }
}
